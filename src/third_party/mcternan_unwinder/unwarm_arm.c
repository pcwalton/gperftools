...
UnwResult UnwStartArm(UnwState * const state)
{
    ...
    do
    {
        ...
        /* Rust morestack prologue coprocessor instruction */
        else if(instr == 0xee1d4f70)
        {
            /* Hack: Skip the next 10 instructions. (We know there are 10 because this is done at
             * LLVM frame lowering time.) */
            UnwPrintd1("MRC 15, 0, r4, cr13, cr0, {3} (Rust morestack prologue)");
            state->regData[15].v += 10 * 4;
        }
        /* Data processing */
